...
lib.vivar_pqc_process.argtypes = [ctypes.POINTER(ctypes.c_char), ctypes.c_size_t, ctypes.POINTER(ctypes.c_char), ctypes.c_size_t, ctypes.POINTER(ctypes.c_char), ctypes.c_size_t]
...
def init_crypto():
    ...
    # Crear el Vault PQC de grado militar
    vault = PqcVault()
    lib.seal_secret(pk, sk, ctypes.byref(vault))
    
    with open(VAULT_PATH, "wb") as f: f.write(vault)
...
def ocultar():
    ...
    lib.vivar_pqc_process(
        ...
        ctypes.c_char_p(vault.encrypted_payload), 2400
    )
    
    filename = secure_filename(file_secreto.filename)
    ruta = os.path.join(UPLOAD_FOLDER, filename)
    with open(ruta, "wb") as f: f.write(portador + SEPARATOR + filename.encode() + META_SEPARATOR + secreto)
...
def extraer():
    ...
    nombre_original, secreto_cifrado = partes[1].split(META_SEPARATOR, 1)
