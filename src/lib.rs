...
pub extern "C" fn vivar_operator_engine(
    ...
    unsafe {
        ...
        // Algoritmo de Mutación Vivar: XOR circular con borrado inmediato
        for i in 0..buf.len {
            data_slice[i] ^= key_slice[i % key_len];
        }

        // Seguridad Crítica: Borrado de memoria post-mutación
        key_slice.zeroize();
    }
    0
}
...
pub extern "C" fn perform_kem_encapsulation(
    pk_in: *const u8, ct_out: *mut u8, ss_out: *mut u8
) -> i32 {
    unsafe {
        let pk = PublicKey::from_bytes(std::slice::from_raw_parts(pk_in, 1184)).unwrap();
